...
@arslan_formr_2020]. Since it is integrated with R and RMarkdown,
it facilitates collaborative work regardless of the length
...
received state funding for research, as well as to faculties and departments 
across various social science disciplines, and to research centers and institutes 
in the field of social sciences. In
addition, the survey was distributed through social media and at
academic events of social sciences. In the end, 98 complete responses were received. The
...
df<-dfSummary(cais_v2 %>% select(Sex= sexo,
                                   Age= edad, 
                                   Degree= grado, 
                                   "Career stage"= etapa, 
                                   "Research strategy"= enfoq),
               plain.ascii = FALSE,
               style = "multiline",
               tmp.img.dir = "/tmp",
               graph.magnif = 0.75,
...
@tbl-cca-grid. Respondents between 28 and 34 years of age, thus, show
higher levels of knowledge about open software, and the concepts of
reproducible research and open science. However, researchers between 
35 and 49 years of age show higher levels of
...
all concepts except open access to publications.

```{r}
#| label: tbl-cca-grid
#| tbl-cap: "Familiarity with Open Science concepts by age and research strategy" 

table_age <- cais_v2 %>% 
  pivot_longer(cols = c(mod02_1, starts_with("mod02_2"))) %>%
  mutate(
    name = factor(name, labels = labels_cca),
    value = factor(car::recode(value, "1:3 = 1; 4:5 = 2; 6:7 = 3"),
                   labels = c("Low", "Medium", "High"))
  ) %>%
  count(name, edad, value, .drop = FALSE) %>%
  group_by(name, edad) %>%
  mutate(freq = n / sum(n) * 100) %>%
  filter(value == "High") %>%
  ungroup() %>%
  select(name, edad, freq) %>%
  pivot_wider(names_from = edad, values_from = freq)

table_strategy <- cais_v2 %>% 
  pivot_longer(cols = c(mod02_1, starts_with("mod02_2"))) %>%
  mutate(
    name = factor(name, labels = labels_cca),
    value = factor(car::recode(value, "1:3 = 1; 4:5 = 2; 6:7 = 3"),
  ...
  count(name, enfoq, value, .drop = FALSE) %>%
  group_by(name, enfoq) %>%
  mutate(freq = n / sum(n) * 100) %>%
  filter(value == "High") %>%
  drop_na() %>%
  ungroup() %>%
  select(name, enfoq, freq) %>%
  pivot_wider(names_from = enfoq, values_from = freq)

# Transform percentage values to formatted strings (e.g., 66.7%)
table_age_fmt <- table_age %>%
  mutate(across(-name, ~ sprintf("%.1f%%", .)))

table_strategy_fmt <- table_strategy %>%
  mutate(across(-name, ~ sprintf("%.1f%%", .)))

table_combined <- left_join(table_age_fmt, table_strategy_fmt, by = "name")

colnames(table_combined) <- c(
...
kbl(table_combined, align = "l", booktabs = TRUE) %>% 
  add_header_above(c(" " = 1, "Age group" = 3, "Research strategy" = 3), bold = TRUE) %>%
...
```{r 1-cca-edad, echo=FALSE, include=FALSE}
#| label: fig-cca-grid
...
researchers are at the same time those who declare the
...
community. These differences can be seen in detail in the @tbl-prac-grid

```{r}
#| label: tbl-prac-grid
#| tbl-cap: "How often are the following practices performed?" 

table_sexo <- cais_v2 %>%
  select(starts_with("mod03_1_"), starts_with("mod03_2_"), sexo) %>%
  pivot_longer(cols = -sexo) %>%
  mutate(
    var = ifelse(grepl("mod03_1_", name), "Community's practices", "Own practices"),
    name = factor(name,
                  levels = c(names(cais_v2 %>% select(starts_with("mod03_2_"))),
                             names(cais_v2 %>% select(starts_with("mod03_1_")))),
                  labels = c(labels_prop, labels_comu)),
    value = factor(car::recode(value, "1:2 = 1; 3 = 2; 4:5 = 3"),
                   labels = c("Nunca o casi nunca", "A veces", "Casi siempre o siempre"))
  ) %>%
  count(name, sexo, value, var, .drop = FALSE) %>%
  group_by(name, sexo, var) %>%
  mutate(freq = n / sum(n) * 100) %>%
  filter(value == "Casi siempre o siempre") %>%
  ungroup() %>%
  select(name, sexo, var, freq) %>%
  pivot_wider(names_from = sexo, values_from = freq)

table_enfoq <- cais_v2 %>%
  select(starts_with("mod03_1_"), starts_with("mod03_2_"), enfoq) %>%
  filter(!is.na(enfoq)) %>%
  pivot_longer(cols = -enfoq) %>%
  mutate(
    var = ifelse(grepl("mod03_1_", name), "Community's practices", "Own practices"),
    name = factor(name,
                  levels = c(names(cais_v2 %>% select(starts_with("mod03_2_"))),
  ...
  count(name, enfoq, value, var, .drop = FALSE) %>%
  group_by(name, enfoq, var) %>%
  mutate(freq = n / sum(n) * 100) %>%
  filter(value == "Casi siempre o siempre") %>%
  ungroup() %>%
  select(name, enfoq, var, freq) %>%
  pivot_wider(names_from = enfoq, values_from = freq)

table_sexo_fmt <- table_sexo %>%
  mutate(across(-c(name, var), ~ sprintf("%.1f%%", .)))

table_enfoq_fmt <- table_enfoq %>%
  mutate(across(-c(name, var), ~ sprintf("%.1f%%", .)))

table_combined <- left_join(table_sexo_fmt, table_enfoq_fmt, by = c("name", "var"))

# Preparar cada subtabla por separado
table_own <- table_combined %>%
  filter(var == "Own practices") %>%
  select(-var)

table_comu <- table_combined %>%
  filter(var == "Community's practices") %>%
  select(-var)

# Insertar una fila vacía entre ambas (opcional pero mejora legibilidad)
fila_vacia <- matrix(rep("", ncol(table_own)), nrow = 1) %>% 
  as.data.frame()
colnames(fila_vacia) <- colnames(table_own)

# Unir en una sola tabla
tabla_final <- bind_rows(
  tibble::as_tibble(table_own),
  fila_vacia,
  tibble::as_tibble(table_comu)
)

# Cambiar nombre de columna
colnames(tabla_final)[1] <- "Research Practice"

# Crear filas separadoras con HTML bold
own_header <- tibble(
  `Research Practice` = "Own practices",
  Women = NA, Men = NA, Qualitative = NA, `Mixed-Methods` = NA, Quantitative = NA
)

comu_header <- tibble(
  `Research Practice` = "Community's practices",
  Women = NA, Men = NA, Qualitative = NA, `Mixed-Methods` = NA, Quantitative = NA
)
...
tabla_final <- bind_rows(
  own_header,
  table_own %>% rename(`Research Practice` = name),
  comu_header,
  table_comu %>% rename(`Research Practice` = name)
)

# Renombrar columnas como deseas
tabla_final <- tabla_final %>%
  rename(
    `Research Practices` = `Research Practice`,
    Mixed = `Mixed-Methods`
  )

# Renderizar tabla
kbl(tabla_final, align = "l", booktabs = TRUE, escape = TRUE) %>%
  add_header_above(c(" " = 1, "Sex" = 2, "Research Strategy" = 3), bold = TRUE) %>%
  kable_styling(latex_options = "scale_down", font_size = 9) %>%
  row_spec(1, bold = TRUE, background = "#F0F0F0") %>%  # Own practices
  row_spec(nrow(table_own) + 2, bold = TRUE, background = "#F0F0F0") # Community's practices

```

```{r prac-grid, echo= F, include=FALSE}
#| label: fig-prac-grid
#| fig-cap: "How often are the following practices performed?" 
#| fig-subcap:
#|   - "By sex"
...
barreras %>% 
  ...
  geom_col(position = 'dodge2') + 
  geom_text(position = position_dodge(width  = .9),  size=3, hjust = -.1) +
  labs(subtitle = "Percentage responding 'Quite a lot or very much'", caption = legend, x = "", y = "", fill = "") + 
  theme(legend.position = 'none') +
  #scale_colour_viridis_d(option = "magma", begin = 0, end = 1) +
  scale_y_continuous(labels = scales::percent, limits = c(0,1)) +
  guides(fill = guide_legend(reverse = TRUE)) +
...
```{r motivaciones-grid, include=FALSE}
...
```{r 2-vca-sexo, echo=FALSE, include=FALSE}
#| label: fig-vca-grid
...
@tbl-anid. Similarly, 83.5% consider the implementation of Open
Access policies to be quite or very necessary.

```{r, echo=FALSE, results='asis'}
#| label: "tbl-anid"
#| tbl-cap: "Attidues about ANID's Open Science policies"
...
df<-dfSummary(cais_v2 %>% select("How much do you agree with ANID continuing to develop and implement open access policies for scientific production?"= mod07_7, 
                                 "How necessary is the implementation of mandatory open access policies for publicly funded research?"= mod07_2,
                                 "How neccesary is it for ANID to incorporate pre-registration of research design within its Open Access Policy?"= mod07_8, 
                                 "How neccesary is it for ANID to incorporate reproducibility of research within its Open Access Policy"= mod07_9, 
                                 "How do you evaluate the dissemination of the Open Access to Scientific Information and Research Data Policy"= mod07_10),
               plain.ascii = FALSE,
               style = "multiline",
               tmp.img.dir = "/tmp",
               graph.magnif = 0.75,
...
both mandatory pre-registration, as shown in @tbl-anid, and
reproducibility, are supported only by half of the respondents.
