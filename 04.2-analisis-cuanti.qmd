...
```{r, echo=FALSE}
cais_summary <- dfSummary(cais_v2 %>% select(edad, grado, etapa, enfoq), headings = FALSE, graph.col = FALSE)
...
ggplot(aes(x = 2, y = freq, fill = mod05_3)) + 
    ...
    coord_polar(theta = "y") +
    xlim(0.5, 2.5) + # PARA EL TAMANO
    scale_fill_brewer(palette = "Pastel1") + # REVISAR COLORES
    labs(
      title = "Percepción sobre la crisis de reproducibilidad",
      subtitle = "Distribución de respuestas",
      fill = "Responde") +
    theme_minimal() +
...
cais_v2 %>%
  mutate(mod05_3 = factor(mod05_3, 
         ...
         enfoq0 = factor(enfoq0,
                          levels = c(1, 2, 3),
                          labels = c("Cualitativo", "Mixto", "Cuantitativo"))) %>%
  filter(!is.na(mod05_3), !is.na(enfoq0)) %>%  #Elimina valores NA
  group_by(enfoq0, mod05_3) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  complete(enfoq0, mod05_3, fill = list(n = 0)) %>%  # rellenar vacios
  group_by(enfoq0) %>% 
  mutate(freq = n / sum(n),
  ...
  labs(
    title = "Percepción sobre la crisis de reproducibilidad por enfoque",
    subtitle = "Distribución de respuestas (na borrados)",
    fill = "Responde"
  ) +
  theme_minimal() +
  theme(
  ...
  geom_text(aes(label = porcentaje), 
            ...
            color = "black", size = 4) # Ajusta texto
...
cais_v2 %>%
  ...
  summarise(n = n()) %>%
  mutate(acum = cumsum(n / sum(n))) %>%
  ggplot(aes(x = mod05_4, y = acum)) +
  geom_line() +
  labs(title = "Gráfico de Líneas: Acumulado de Proporciones",
       x = "Porcentaje percibido de trabajos reproducibles (?",
       y = "Acumulado Proporcional") +
  theme_minimal()
...
cais_v2 %>%
  ...
  group_by(name) %>%
  mutate(freq = n / sum(n),
         porcentaje = paste0(round(freq * 100, 1), "%")) %>% # Calcula los porcentajes
  ggplot(aes(x = 2, y = freq, fill = value)) +
  geom_col(width = 1, color = "white") + # Gráfico de dona
  coord_polar(theta = "y") +
  scale_fill_manual(values = c("#FF9999", "#FFCC99", "#99CCFF")) + # Personaliza los colores
  theme_void() + # Elimina el fondo y los ejes
  xlim(0.5, 2.5) + # Crea el efecto de dona
  labs(title = "Distribución de respuestas",
       subtitle = "¿Ha intentado reproducir los resultados de alguien más?",
       fill = "Estado") +
  geom_text(aes(label = porcentaje), # Mapea los porcentajes como etiquetas
            position = position_stack(vjust = 0.5), # etiqueta centrada
            color = "black", size = 4) # texto
