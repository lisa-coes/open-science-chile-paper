...
Como se evidencia en la @fig-dona, el 39% de los encuestados cree que hay una crisis importante de reproducibilidad, mientras que el 40% no sabe si la hay.
...
ggplot(aes(x = 2, y = freq, fill = mod05_3)) + 
  ...
  scale_fill_viridis_d(option = "magma", begin = .1, end = .7, direction = 1) +
    theme(axis.text = element_blank(),
      ...
      plot.subtitle = element_text(hjust = 0.5)) + geom_text(aes(label = porcentaje), 
            position = position_stack(vjust = 0.5), # posicion etiqueta
            color = "white", size = 4)# texto
...
Por otro lado, el 59% cree que el 30% o menos de las investigaciones en ciencias sociales son reproducibles; el 83% cree que menos de la mitad de las investigaciones en ciencias sociales son reproducibles, como se muestra en @tbl-tablaacum.
...
Esto va en línea con que el 40% de los encuestados ha intentado reproducir resultados de otros investigadores (@fig-dona-2-1). De este porcentaje, solo el 20% de los intentos han logrado una reproducción completa, mientras que el 69% ha logrado reproducciones parciales (@fig-dona-2-2).
...
Sin embargo, hay otras prácticas que encuentran una menor acogida. Así, tanto la obligatoriedad del prerregistro, como se muestra en @fig-vanid-grid-2-1, como la de la reproducibilidad, como se observa en @fig-vanid-grid-2-2, obtienen el apoyo del 50% de los encuestados.
```{r}
...
Por último, la política de difusión de la ANID es evaluada negativamente por la mayoría de los encuestados. Solo un 24% considera que esta política es buena o muy buena, como se observa en @fig-vanid-2.
```{r}
#| label: fig-vanid-2
...
ANID2 <- cais_v2 %>% select(mod01_10, mod08_2, mod08_1, mod07_2, mod07_8, mod07_9) %>% #QUE TAN NECESARIA ES LA CS ABIERTA EN ANID
  mutate_at(vars(starts_with("mod07")),
            ~ if_else(. == 1, "Nada", 
...
ANID3 <- cais_v2 %>% select(mod01_10, mod08_2, mod08_1, mod07_7) %>% 
  mutate_at(vars(starts_with("mod07")), 
...
ANID4 <- cais_v2 %>% select(mod01_10, mod08_2, mod08_1, mod07_10) %>% 
  mutate_at(vars(starts_with("mod07")), 
            ~ if_else(. == 1, "Mala", 
                      if_else(. == 2, "Regular", 
                              if_else(. == 3, "Neutra",
...
ANID5 <- cais_v2 %>% select (mod01_10, mod08_2, mod08_1, mod07_3)
